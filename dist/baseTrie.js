...
class Trie {
    constructor(db, root) {
        ...
        this.db = db !== null && db !== void 0 ? db : new mapDb_1.MapDb();
        this._root = this.EMPTY_TRIE_ROOT;
        if (root) {
            this.setRoot(root);
        }
    }
    static async fromProof(proofNodes, proofTrie) {
    ...
    static async verifyProof(rootHash, key, proofNodes) {
    ...
    async batch(ops) {
        var e_1, _a;
        try {
            for (var ops_1 = __asyncValues(ops), ops_1_1; ops_1_1 = await ops_1.next(), !ops_1_1.done;) {
                const op = ops_1_1.value;
                if (op.type === 'put') {
                    if (!op.value) {
            ...
            try {
                if (ops_1_1 && !ops_1_1.done && (_a = ops_1.return)) await _a.call(ops_1);
            }
